...
export class MapHocComponent {
  ...
  private map: any;
  private pixelsInGameUnit: number;
...
throw new Error('Method not implemented.');
  ...
  private loadMap(gameData: MapHoc, gameConfig: MapConfig): void {
    ...
    this.gamedata.widthInMeters = 8000;
    this.gamedata.heightInMeters = 8000;
    ...
    if (gameConfig.globalMapFileName && gameConfig.mapBounds) {
      let b = [
        this.convertGameCoorsToMapCoors(gameConfig.mapBounds[0][0], gameConfig.mapBounds[0][1]),
        this.convertGameCoorsToMapCoors(gameConfig.mapBounds[1][0], gameConfig.mapBounds[1][1])
      ]
      L.imageOverlay(`/assets/images/maps/hoc/${gameConfig.globalMapFileName}`, b).addTo(this.map);
    }
    ...
    if (gameConfig.rulerEnabled) {
      ruler = this.mapService.addRuler(this.map, this.pixelsInGameUnit, 8000);
    }
    ...
    this.translate.onLangChange.subscribe(i=>{
      ...
      let layersToControl = layersToLayerController.map((x: any) => [this.translate.instant(x.name), x]);
  ...
  private addMarkers(): void {
    ...
    },
    {
      ...
      icon: new this.svgIcon({
        iconUrl: '/assets/images/s2/Markers/Texture_Archianomaly_NotActive_General_Shadow.png',
        iconAnchor: [0, 0],
      }),
    },
    {
      ...
      icon: new this.svgIcon({
        iconUrl: '/assets/images/s2/Markers/Texture_Camp_NotActive_General_Shadow.png',
        iconAnchor: [0, 0],
      }),
    }];
    ...
    for (let data of this.gamedata.markers) {
      ...
      let marker = new this.svgMarker(this.convertGameCoorsToMapCoors(data.z, data.x), {renderer: this.canvasRenderer, icon: icon, radius: 40});
      ...
      if (data.radius > 0) {
        ...
        circleMarkers.push(L.circle(this.convertGameCoorsToMapCoors(data.z, data.x), {radius: data.radius * this.pixelsInGameUnit, color: color, weight: 2}));
      }
