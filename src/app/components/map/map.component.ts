...
export class MapComponent {
  ...
  private addTraders() {
    let traderIcon = { name: this.translate.instant("traders"), uniqueName: 'traders', cssClass: "tradere", ableToSearch: true, icon: L.icon({ iconSize: [4, 4], className: 'mark-container stalker-mark-2', animate: false, iconUrl: '/assets/images/svg/marks/trader.svg', iconSizeInit: [2, 2], iconAnchor: [0, 0] }) };
    ...
    for (let trader of this.gamedata.traders) {
        ...
        this.createProperty(canvasMarker.feature.properties, "search", [trader.name + '.name'], this.translate);

        let location = this.locations.locations.find((y: { id: any; }) => y.id == trader.locationId);
        canvasMarker.properties.locationUniqueName = location.uniqueName;
